...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.videoMaskView = [[SAVideoMaskView alloc]initWithFrame:frame playBtnClick:^(UIButton *playBtn) {
            [weakself playBtnClick:playBtn];
        } fullScreenBtnClick:^(UIButton *fullScreenBtn) {
            [weakself fullScreenClick:fullScreenBtn];
        } backBtnClick:^(UIButton *backBtn) {
            [weakself backClick:backBtn];
        }];
...
- (void)fullScreenClick:(UIButton *)btn
{
    ...
    if (btn.selected) {
        ...
        self.videoMaskView.frame = self.window.bounds;
        ...
        self.videoMaskView.frame = self.smallFrame;
        _ButtonAction(btn);
    }
}
#pragma mark -- 返回按钮
...
{
    if (self.videoMaskView.fullScreenBtn.selected) {
        [UIDevice setOrientation:UIInterfaceOrientationPortrait];
        self.frame = self.smallFrame;
        self.videoMaskView.frame = self.smallFrame;
    ...
    [self.videoMaskView.fullScreenBtn setSelected:NO];
}
    ...
    if (object == self.playerItem) {
        if ([keyPath isEqualToString:@"status"]) {
            if (self.player.status == AVPlayerStatusReadyToPlay) {
                [self.videoMaskView.activityView startAnimating];
                self.videoMaskView.backgroundImageView.hidden = YES;
                [self play];
