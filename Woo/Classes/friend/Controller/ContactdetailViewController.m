...
-(void)deletefriend
{
    ...
    [self showhudmessage:@"请求处理中..." offy:-100];
    ...
    [[UserRequestToo shareInstance] statrrequestgetwith:MHAsiNetWorkPOST successBlock:^(id returnData) {
        ...
        if (returnData[@"success"]) {
            if([returnData[@"status"] isEqualToString:@"200"]) {
                [self showAlertHud:self.view.center withStr:@"删除好友成功" offy:-100];
                [[NSNotificationCenter defaultCenter] postNotificationName:UPDATE_GROUP_FRIEND object:nil];
                [weakSelf performSelector:@selector(popcontrolle) withObject:nil afterDelay:1.5];
                return ;
            }
            else{
        ...
        [self hideHud];
        [self showAlertHud:self.view.center withStr:@"删除好友失败" offy:-100];
    }];
}
-(void)popcontrolle
{
...
-(void)tochat
{
     [[ChatmangerObject ShareManger] gotoChatViewtargetid:self.userinfo.userId convertitle:self.userinfo.displayName conversationModelType:ConversationType_PRIVATE fromview:self];
}
