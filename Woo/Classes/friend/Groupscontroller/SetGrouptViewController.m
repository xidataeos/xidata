...
#define BTN_TAG 336
@interface SetGrouptViewController ()<UITextFieldDelegate,UITextViewDelegate>
...
-(void)setBtnClick
{
    ...
    if (self.groupnamefield.text.length==0) {
        [self.view endEditing:YES];
        [self showAlertHud:self.view.center withStr:@"群组名称还是空的哦!" offy:-100];
        return;
    }
    if (self.jianjiefield.text.length==0) {
         [self.view endEditing:YES];
        [self showAlertHud:self.view.center withStr:@"群简介称还是空的哦!" offy:-100];
        return;
    }
    [self.dataArray removeAllObjects];
    [self showhudmessage:@"数据处理中..." offy:-100];
    ...
    [UserRequestToo shareInstance].rquesturl=Modifythegroup;
    ...
    [[UserRequestToo shareInstance] statrrequestgetwith:MHAsiNetWorkPOST successBlock:^(id returnData) {
        ...
        [self showAlertHud:self.view.center withStr:@"群信息设置失败" offy:-100];
        [self hideHud];
    }];
}
...
-(UITextField *)groupnamefield
{
    if (_groupnamefield==nil) {
        _groupnamefield=[[UITextField alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.useriamge.frame)+15*KWidth_Scale,62*KWidth_Scale, SCREEN_WIDTH-55,20*KWidth_Scale)];
        _groupnamefield.placeholder=@"请输入社群名称";
        _groupnamefield.textAlignment=NSTextAlignmentLeft;
        _groupnamefield.textColor=RGB(153, 153, 153);
...
-(UIImageView *)useriamge
{
    if (_useriamge==nil) {
        ...
        _useriamge.image=[UIImage imageNamed:@"photo_boy"];
    }
    return _useriamge;
}
-(void)settopview
