...
+ (CGSize)sizeForMessageModel:(RCMessageModel *)model
    ...
    TransferMessage *message = (TransferMessage *)model.content;
    CGSize size = [TransFercell getBubbleBackgroundViewSize:message];
...
- (void)initialize {
    ...
    [self.detaillable setTextAlignment:NSTextAlignmentLeft];
    ...
    self.tiplabel.text=@"WoWo转账";
    [self.bubbleBackgroundView addSubview:self.tiplabel];
    
    [self.bubbleBackgroundView addSubview:self.titleimage];
    [self.bubbleBackgroundView addSubview:self.textLabel];
    [self.bubbleBackgroundView addSubview:self.detaillable];
...
- (void)setAutoLayout {
    TransferMessage *testMessage = (TransferMessage *)self.model.content;
    if (testMessage) {
        self.textLabel.text = [NSString stringWithFormat:@"¥ %@",testMessage.asset];
    }
     if (MessageDirection_RECEIVE == self.messageDirection) {
         self.detaillable.text = [NSString stringWithFormat:@"已到账"];
     }
     else{
         self.detaillable.text = [NSString stringWithFormat:@"转账提醒"];
     }
    ...
    CGRect messageContentViewRect = self.messageContentView.frame;
     //拉伸图片
    if (MessageDirection_RECEIVE == self.messageDirection) {
        self.titleimage.frame=CGRectMake(10*KWidth_Scale, (SCREEN_WIDTH*0.4/2.6-25*KWidth_Scale)/2.0, 25*KWidth_Scale, 25*KWidth_Scale);
        self.titleimage.image=[UIImage imageNamed:@"pic_transfer"];
        self.textLabel.frame = CGRectMake(48*KWidth_Scale,8*KWidth_Scale, textLabelSize.width, textLabelSize.height/3.3);
        self.detaillable.frame= CGRectMake(48*KWidth_Scale,CGRectGetMaxY(self.textLabel.frame), textLabelSize.width, textLabelSize.height/2.5);
        
            self.detaillable.text = [NSString stringWithFormat:@"转账提醒"];
        self.tiplabel.frame=CGRectMake(5*KWidth_Scale,CGRectGetMaxY(self.detaillable.frame)+5*KWidth_Scale, textLabelSize.width, 18*KWidth_Scale);
...
//        }
        ...
        UIImage *image = [RCKitUtility imageNamed:@"bg_to_transfer" ofBundle:@"JResource.bundle"];
        ...
        self.textLabel.frame = CGRectMake(45*KWidth_Scale,8*KWidth_Scale, textLabelSize.width, textLabelSize.height/3.3);
        self.detaillable.frame= CGRectMake(48*KWidth_Scale,CGRectGetMaxY(self.textLabel.frame), textLabelSize.width, textLabelSize.height/2.5);
        self.tiplabel.frame=CGRectMake(5*KWidth_Scale,CGRectGetMaxY(self.detaillable.frame)+5*KWidth_Scale, textLabelSize.width, 18*KWidth_Scale);
        self.textLabel.textColor=[UIColor whiteColor];
        messageContentViewRect.size.width = bubbleBackgroundViewSize.width;
        ...
        self.bubbleBackgroundView.frame =
        CGRectMake(0, 0, bubbleBackgroundViewSize.width, bubbleBackgroundViewSize.height);
        UIImage *image = [RCKitUtility imageNamed:@"bg_from_transfer" ofBundle:@"JResource.bundle"];
        self.bubbleBackgroundView.image =
        [image resizableImageWithCapInsets:UIEdgeInsetsMake(image.size.height, image.size.width,
                                                            image.size.height, image.size.width)];
    }
...
+ (CGSize)getTextLabelSize:(TransferMessage *)message {
