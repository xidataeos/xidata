...
RealTimeLocationStatusViewDelegate,UIAlertViewDelegate,SendRedpocketDelegate,TransFerDelegate>
...
object:nil];
 ...
 selector:@selector(onEndForwardMessage:)name:@"RCDForwardMessageEnd"object:nil];
    if (self.conversationType==ConversationType_PRIVATE) {
        ...
        [pluginBoardView removeItemAtIndex:4];
...
-(void)getaredenvelope:(RedMessage *)model redmodle:(RCMessageModel*)redmodle
{
    ...
    [[UserRequestToo shareInstance] statrrequestgetwith:MHAsiNetWorkPOST successBlock:^(id returnData) {
        if (returnData[@"success"]) {
            ...
            {
                ReadPocketview *red=[[ReadPocketview alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT) retype:red_type_have withtips:@""];
                [red shareviewShow];
                [red setMyblock:^{
                ...
                ReadPocketview *red=[[ReadPocketview alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT) retype:red_type_no withtips:tips];
                [red setMyblock:^{
                    [self receiveRedmessage:redmodle isgroup:NO];
                }];
                [red shareviewShow];
            }
        }
...
-(void)getGroupelope:(RedMessage *)model redmodle:(RCMessageModel*)redmodle receiveid:(NSString *)receiveid
{
    WEAKSELF;
    NSMutableDictionary *parametDic = [NSMutableDictionary dictionaryWithCapacity:9];
    [parametDic setObject:receiveid forKey:@"recvId"];
    ...
    [[UserRequestToo shareInstance] statrrequestgetwith:MHAsiNetWorkPOST successBlock:^(id returnData) {
        if (returnData[@"success"]) {
            if([returnData[@"status"] isEqualToString:@"200"])
            {
                ...
                ReadPocketview *red=[[ReadPocketview alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT) retype:red_type_no withtips:tips];
                [red setMyblock:^{
                    [self receiveRedmessage:redmodle isgroup:YES];
                }];
                [red shareviewShow];
            }
        }
...
- (void)didTapCellPortrait:(NSString *)userId {
    [self getFriendRelationship:self.targetId istapPortrait:YES];
}
//查看好友关系
-(void)getFriendRelationship:(NSString *)userId istapPortrait:(BOOL)istapPortrait
{
    ...
    [parametDic setObject:userId forKey:@"fid"];
                ...
                if([returnData[@"status"] isEqualToString:@"200"]) {
                    if ([dic[@"isFriend"] isEqual:@1]) {
                        ...
                        if (istapPortrait) {
                            ...
                            RCPluginBoardView *pluginBoardView = self.chatSessionInputBarControl.pluginBoardView;
                            [pluginBoardView insertItemWithImage:[RCKitUtility imageNamed:@"ic_transfer" ofBundle:@"JResource.bundle"] title:@"转账" atIndex:4 tag:PLUGIN_BOARD_ITEM_ZhuanZhang_TAG];
                        }
