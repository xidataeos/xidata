...
function Redenvelop() {
    ...
    this.recv = function (user, userPK, asset, memo) {
        eos = Eos(config)
        eos.contract("redenvelope").then(redenvelop => {  //hello随便起的变量名
            redenvelop.recv(user, userPK, asset, memo, {                         //hi是方法名, 'axay'是该hello合约hi方法的参数
    ...
    this.send = function (user,userPK,asset, num, memo) {
        eos = Eos(config)
        eos.contract("redenvelope").then(redenvelop => {  //hello随便起的变量名
            redenvelop.send(user, userPK, asset, num,memo, {                         //hi是方法名, 'axay'是该hello合约hi方法的参数
                authorization: ['redenvelope']        //testtesttest是建立该合约的用户
            }).then(result => {
                console.log(JSON.stringify(result));
                res.writeHead(200, {'Content-Type': 'application/json'});
