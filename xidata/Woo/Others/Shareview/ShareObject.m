    ...
    switch (platform) {
        case share_weChat:
        {
            if (![WXApi isWXAppInstalled]) {
                [fromecontroller showInfoWithStatus:@"还没安装微信" ];
                return;
            }
            else{
        ...
        case share_Qzone:
        {
            if (![ShareSDK isClientInstalled:SSDKPlatformTypeQQ]) {
                [fromecontroller showInfoWithStatus:@"还没安QQ" ];
    ...
    [ShareSDK share:shareType
     ...
     onStateChanged:^(SSDKResponseState state, NSDictionary *userData, SSDKContentEntity *contentEntity, NSError *error) {
         ...
         if (state == SSDKResponseStateSuccess) {
             [fromecontroller showInfoWithStatus:@"分享成功" ];
             //统计分享
             if (shareType == SSDKPlatformTypeFacebook) {
              ...
              [fromecontroller showInfoWithStatus:@"分享失败" ];
