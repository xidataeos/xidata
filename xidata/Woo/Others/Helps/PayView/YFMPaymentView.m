...
-(UITableView *)tableView {
    if (!_tableView) {
        ...
        self.titleLable = [[UILabel alloc] initWithFrame:CGRectMake(15, 0, KScreenWidth-30, 50)];
        self.titleLable.textAlignment = 0;
        self.titleLable.text = [NSString stringWithFormat:@"支付金额: ¥ %@",self.totalBalance];
        self.titleLable.font = [UIFont systemFontOfSize:14];
        self.titleLable.textColor = CColor;
        self.titleLable.numberOfLines = 1;
        if (self.selftype==selecttype) {
            self.titleLable.text=@"选择账户";
        }
        else{
            self.titleLable.attributedText = [self setPriceAttreWithStr:self.titleLable.text];
        }
        [v addSubview:self.titleLable];
...
-(void)creatSureBtn
{
    ...
    [self.sureBtn setBackgroundColor:RemindRedColor];
