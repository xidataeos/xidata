...
@interface WooSpotViewController ()
...
@implementation WooSpotViewController
...
- (void)requestURL
{
    if (self.textField2.text.length!=11) {
        [self showAlertHud:self.view.center withStr:@"手机号码不对" offy:-100];
        return;
    }
    WEAKSELF;
    [self showhudmessage:@"加载中..." offy:-100];
    ...
    [[UserRequestToo shareInstance] statrrequestgetwith:(MHAsiNetWorkPOST) successBlock:^(id returnData) {
        ...
        if ([returnData[@"success"] intValue] == 1) {
            [self showAlertHud:self.view.center withStr:@"您已报名成功" offy:-100];
        } else {
