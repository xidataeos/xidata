...
@property (weak, nonatomic) IBOutlet UIButton *exceptionalButton;//打赏
...
+ (instancetype)sharedInstance {
    static audioViewController *_sharedMusicVC = nil;
...
- (void)setupBackgroudImage {
    ...
    [_backgroudImageView sd_setImageWithURL:[NSURL URLWithString:@""] placeholderImage:[UIImage imageNamed:@"music_placeholder"]];
...
-(void)didDismissself:(UIPanGestureRecognizer *)sender
{
    ...
    swipingPoint = [sender locationInView:self.view];
    ...
    {
        if (self.view.frame.origin.y >SCREEN_WIDTH*0.6) {
            __weak typeof(self) weakSelf = self;
            [UIView animateWithDuration:0.3 animations:^{
                self.view.frame = CGRectMake(0,SCREEN_HEIGHT, viewWidth, viewHeigh);
            ...
            self.view.frame = CGRectMake(0,0, viewWidth, viewHeigh);
        }
    }
}
- (IBAction)didTouchDismissButton:(id)sender {
    [self.navigationController dismissViewControllerAnimated:YES completion:^{
        self.dontReloadMusic = NO;
        self.lastMusicUrl = [self currentPlayingMusic].musicUrl.mutableCopy;
    }];
}
