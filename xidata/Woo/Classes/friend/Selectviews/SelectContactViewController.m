...
-(void)setBtnClick
{
    if(self.selectcontacts.count!=0)
    {
        ...
        [self showInfoWithStatus:@"还没有任何选择"];
        return;
    }
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.selectcontacts.count!=0) {
        [self.selectcontacts enumerateObjectsUsingBlock:^( RCDUserInfo* obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [newcell setMyblock:^(BOOL selectstatus) {
        if (selectstatus) {
               [weakSelf.selectcontacts addObject:model];
        }
        else{
               [weakSelf.selectcontacts removeObject:model];
        }
        if (weakSelf.selectcontacts.count!=0) {
            self.title=[NSString stringWithFormat:@"邀请社群成员数 %lu",(unsigned long)weakSelf.selectcontacts.count];
        }
    }];
    return newcell;
}
...
- (void)searchBarTextDidBeginEditing:(UISearchBar *)searchBar {
    ...
    searchViewController.searchlist=self.selectcontacts;
