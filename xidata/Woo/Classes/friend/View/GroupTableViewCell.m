...
-(UILabel *)namelabel
{
    if (_namelabel==nil) {
        ...
        _namelabel.frame=CGRectMake(CGRectGetMaxX(self.avrimage.frame)+10, 10, ScreenWidth*0.6, 20);
    }
    return _namelabel;
}
...
-(void)setdata:(NSString *)unreadcountstr
{
    ...
    if (unreadcountstr.length<=2) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            _tipslabel.bubbleWidth = 33;
        });
        _tipslabel.bubbleLabel.text =@"99+";
        _tipslabel.bubbleLabel.font=UIFont(10);
    }
...
-(void)setModel:(RCConversation *)model
{
    if (model.lastestMessageDirection==1) {
        ...
        self.timeslabel.text=[PublicFunction inputTimeStr:[NSString stringWithFormat:@"%lld",model.sentTime]];
    }
    else{
        self.timeslabel.text=[PublicFunction inputTimeStr:[NSString stringWithFormat:@"%lld",model.receivedTime]];
    }
