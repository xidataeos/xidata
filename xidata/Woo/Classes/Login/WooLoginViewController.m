...
- (void)initWithUI
{
    UIImageView *logo=[[UIImageView alloc] initWithFrame:CGRectMake((SCREEN_WIDTH-60*KWidth_Scale)/2.0, StatusBarAndNavigationBarHeight, 70*KWidth_Scale, 70*KWidth_Scale)];
    logo.image=[UIImage imageNamed:@"logo_icon"];
    logo.layer.cornerRadius=35*KWidth_Scale;
...
self.textField1.clearButtonMode=UITextFieldViewModeWhileEditing;
    ...
    if (![PublicFunction isEmpty:[UserDefaults objectForKey:@"rel"]]) {
        self.textField1.text=[UserDefaults objectForKey:@"rel"];
    }
    ...
    [[UserRequestToo shareInstance]statrrequestgetwith:(MHAsiNetWorkPOST) successBlock:^(id returnData) {
        ...
        if ([returnData[@"success"] intValue] == 1) {
            ...
            [UserDefaults setObject:returnData[@"data"][@"rcToken"] forKey:@"rcToken"];
             [UserDefaults setObject:returnData[@"data"][@"token"] forKey:@"token"];
    ...
    [[UserRequestToo shareInstance]statrrequestgetwith:(MHAsiNetWorkPOST) successBlock:^(id returnData) {
        ...
        if ([returnData[@"success"] intValue] == 1) {
            ...
            [UserDefaults setObject:returnData[@"data"][@"sex"] forKey:@"sex"];
             [UserDefaults setObject:returnData[@"data"][@"invitationCode"] forKey:@"invitationCode"];
//            [[NSNotificationCenter defaultCenter] postNotificationName:@"remove" object:nil];
            WooVCViewController *registVC = [[WooVCViewController alloc]init];
